        ...
        if type(data[batch]) not in [list,dict]:
            ...
            for games in data[batch]:
                ...
                for game in games:
                    ...
                    if type(games[game]) != list:
                        games_json[game] = str(games[game]).encode('ascii')
                    else:
                        card_counter = 0
                        for card_histories in list(games[game]):
...
def main():
    ...
    for z in zips:
        ...
        with open('/home/eduardo/personal_git/HS/hearthscryschema.json') as schemafile:
